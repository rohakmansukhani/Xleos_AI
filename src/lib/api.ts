...
export const apiCall = async (endpoint: string, options: ApiOptions = {}) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (
        response.status === 401 &&
        (endpoint.includes("/user/status") || endpoint.includes("/user/stats"))
      ) {
...
export const authApi = {
  ...
  exchangeCode: (code: string) =>
    apiCall(`/auth/callback?code=${code}`, {
      method: "GET",
      credentials: "include",
    }),
...
export const chatApi = {
  ...
  submitScript: (script: string) =>
    scriptApi.submitScript({ script_text: script }),
  ...
  getFeedbackSummary: (submissionId: string) =>
    scriptApi.getFeedbackSummary(submissionId),
};
