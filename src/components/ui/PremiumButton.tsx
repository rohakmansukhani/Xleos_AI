...
const PremiumButton = memo(
  forwardRef<HTMLButtonElement, PremiumButtonProps>(function PremiumButtonRaw(
    {
      variant = "primary",
      size = "md",
      loading = false,
    ...
    const shimmerStyle = animations.includes("shimmer")
      ? {
          background:
            "linear-gradient(90deg,#fff0,rgba(255,255,255,0.08),#fff0)",
          backgroundSize: "200% 100%",
          backgroundPosition: isMagnetic ? "230% 0" : "-200% 0",
        }
      : {};
      ...
      <>
        {animations.includes("glow") && (
          <motion.div
            ...
            animate={
              isMagnetic
                ? { scale: 1.18, opacity: 0.37 }
                : { scale: 1.07, opacity: 0.16 }
            }
            style={{
              background:
