...
export default function LineByLineTimeline({
  ...
  useEffect(() => {
    console.log('📊 [TIMELINE DEBUG] Submission data:', {
      ...
      lines: submission.lines?.map((line) => ({
        ...
        isBonus: line.line_text?.includes('🌟') || line.line_text?.includes('Trending')
      }))
    });
  }, [submission]);

  const [lineStatuses, setLineStatuses] = useState<Array<'processing' | 'ready' | 'completed'>>(
  ...
  useEffect(() => {
    if (submission.status === 'completed') {
      ...
      setLineStatuses(submission.lines.map((line) => {
        // Check if all videos in this line have feedback
        const allVideosFeedback = line.videos.every(video => 
  ...
  const getLineStatusText = (line: BackendLine, status: string) => {
    switch (status) {
      ...
      case 'completed':
        const feedbackCount = line.videos.filter(v => v.feedback.rating !== null).length;
        const avgRating = line.videos
          .filter(v => v.feedback.rating !== null)
          .reduce((sum, v) => sum + (v.feedback.rating || 0), 0) / feedbackCount;
    ...
    <div className="flex-1 px-7 pt-8 pb-6 overflow-y-auto transition-all duration-300">
      <div className="flex items-center gap-2 mb-7">
        ...
        <span className="ml-3 text-[#d6cef7b2] text-lg">
          {(() => {
            const bonusLines = submission.lines.filter(line =>
      ...
      <div className="flex flex-col gap-6">
        {submission.lines.map((line, idx) => {
          ...
          const isBonus = line.line_text?.includes('🌟') ||
                          line.line_text?.includes('Trending') ||
                          line.line_text?.toLowerCase().includes('context') ||
                          line.search_phrase?.toLowerCase().includes('context');
          ...
          return (
            ...
            >
              <div
                ...
                `}
                onClick={() => lineStatuses[idx] !== 'processing' && onSelectLine(idx, line)}
              ...
              >
                <div className="flex flex-col flex-1">
                  <span className={
                    ...
                    {isBonus ? '🌟 ' : ''}Line {line.line_number}: {line.line_text}
                  </span>
                  ...
                  {line.videos && line.videos.length > 0 && lineStatuses[idx] !== 'processing' && (
      ...
      <div className="flex justify-end mt-10">
        <span className="text-[#adaacd]/70 text-xs select-none">
          Xleos AI Script Timeline • {new Date().toLocaleDateString()} • ID: {submission._id.slice(-8)}
        </span>
      </div>
    </div>
  );
