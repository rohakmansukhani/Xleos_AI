...
const WaitlistModal: React.FC<WaitlistModalProps> = ({
  ...
  function validateForm(): FormErrors {
    const n: FormErrors = {};
    if (!formData.fullName.trim()) n.fullName = "Name is required";
    else if (formData.fullName.length < 2)
      n.fullName = "Name must be at least 2 characters";
    if (!formData.address.trim()) n.address = "Email is required";
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.address))
      n.address = "Please enter a valid email address";
    if (!formData.role) n.role = "Please select your role";
        ...
        >
          <motion.div
            ...
            animate={{
              ...
              scale: 1,
              y: 0,
              transition: { type: "spring", bounce: 0.32, duration: 0.58 },
            }}
            exit={{
              opacity: 0,
              scale: 0.95,
              ...
              >
                    ...
                    >
                      ...
                      {roles.map((role) => (
                        <option
                          key={role.value}
                          value={role.value}
                          className="bg-[#231a37] text-white"
                        >
                          {role.label}
                        </option>
                      ))}
                ...
                <motion.div className="bg-white/3 rounded-xl p-4 mb-2 flex flex-col gap-2">
                  ...
                  <div className="flex items-center gap-2 text-sm text-white/70">
                    ...
                    <span>Get exclusive early access</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-white/70">
                    <Clock size={14} className="text-purple-300" />
                    <span>Be notified first when we launch</span>
                  </div>
                </motion.div>
                <motion.div layout>
                  <PremiumButton
